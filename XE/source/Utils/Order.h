...
class XE_API Order : public XE::Singleton< XE::Order >
{
private:
	struct Private;
...
public:
	template< typename T > static XE::uint64 RegisterOrder( const String& name, const String& desc, const MetaInfoCPtr & parameter )
	{
	...
	template< typename T > static void VisitOrder( XE::Delegate< void( XE::uint64, String, String, const MetaInfoCPtr & ) > val )
	{
...
public:
	...
	static MetaInfoCPtr FindOrderPatameter( XE::uint64 id );

public:
	static XE::uint64 RegisterOrder( XE::uint8 group, const String& name, const String& desc, const MetaInfoCPtr & parameter );

	static XE::uint64 FindOrderID( XE::uint8 group, const String& name );

	static void VisitOrder( XE::uint8 group, XE::Delegate< void( XE::uint64, String, String, const MetaInfoCPtr & ) > val );
